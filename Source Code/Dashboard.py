...
classifier = load_model('Trained_modelnew.h5')			#loading the model
...
def openimg():
	...
	cv2.moveWindow("Image", 1500,214)


def removeFile():
	"""Removes the temp.txt and tempgest directory if any stop button is pressed or application is closed"""
	try:
		os.remove("temp.txt")
	except:
		pass
...
class Dashboard(QtWidgets.QMainWindow):
	def __init__(self):
		...
		self.label_3.setGeometry(120,185,780,441)
		movie.start()
	...
	def createGest(self):
		...
		while True:
			...
			try:
				...
				img2 = cv2.rectangle(frame, (140,50),(285,210), (0,255,0), thickness=2, lineType=8, shift=0)
			except:
			...
			cv2.setWindowProperty("mask",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)
			cv2.resizeWindow("mask",210,210)
			cv2.moveWindow("mask", 870,386)
	...
	def exportFile(self):
		...
		self.pushButton_3.clicked.connect(lambda:abc(str(content)))
